...
def test_ddr3(data_num):
    ...
    write_data_into_ddr3(1, 0x0000000, 0x000000a)   # set write begin address and post trigger address and wrap around
    ...
    time.sleep(3600*16)                             # delay 16 hours
    cmd_interpret.write_config_reg(0, 0x0000)       # write disable fifo32to256
    time.sleep(3)
    read_data_from_ddr3(0x000000a)                  # set read begin address

    data_out = []
    ## memoryview usage
    for i in range(data_num):
        data_out += cmd_interpret.read_data_fifo(10)           # reading start
        time.sleep(0.01)
    return data_out
#--------------------------------------------------------------------------#
...
def main():
    ...
    userdefinedir_log = "P15_PhaseAdj=100_QSel=20fC_QInj=1M25_B2_log"
    ...
    for DAC_Pixel in [470]:
    # for DAC_Pixel in range(625, 725):
    # for DAC_Pixel0 in np.arange(400,500,5):
    # for PhaseAdj1 in range(100, 210, 1):
        # QInjection Setting
        QSel = 20
        ...
        hitflag_threshold(100, 0x0001)              # first param for hitflag_threshold, second param for mode select, 0: Continuous mode 1: non-continuous mode
